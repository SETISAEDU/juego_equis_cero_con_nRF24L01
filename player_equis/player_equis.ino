...
//Se crean las direcciones para los pipes 
...
void loop() {
  ...
  if (player == 0){ //Si el jugador activo es equis
    ...
    if (Touch.Pressed()){ // Verificacion si se ha hecho presion en el panel touch
      ...
      if ((Y_Coord > 0) && (Y_Coord < 79)){
        /* Fila 1, columna 1 */
        if (((X_Coord > 0) && (X_Coord < 106) && busy_vector[0] == 0)){ //Coordenadas segmento 0
            player = 1;
            X_vector[0] = 1;
        }
  
        /* Fila 1, columna 2 */
        else if ((X_Coord > 106) && (X_Coord < 211) && busy_vector[1] == 0){ //Coordenadas segmento 1
            player = 1;
            X_vector[1] = 1;
          }
  
       /* Fila 1, columna 3 */
        else if ((X_Coord > 211) && (X_Coord < 319) && busy_vector[2] == 0){ //Coordenadas segmento 2
            player = 1;
            X_vector[2] = 1;
          }
      }
      ...
      {
        /* Fila 2, columna 1 */
        if ((X_Coord > 0) && (X_Coord < 106) && busy_vector[3] == 0){ //Coordenadas segmento 3
            player = 1;
            X_vector[3] = 1;
          }
      
        /* Fila 2, columna 2 */
        else if ((X_Coord > 106) && (X_Coord < 211) && busy_vector[4] == 0){ //Coordenadas segmento 4
            player = 1;
            X_vector[4] = 1;
          }
        /* Fila 2, columna 3 */
        else if ((X_Coord > 211) && (X_Coord < 319) && busy_vector[5] == 0){ //Coordenadas segmento 5
            player = 1;
            X_vector[5] = 1;
        }
      }
      ...
      /* ------------------------------------ Fila 3 ------------------------------------ */
      else if ((Y_Coord > 160) && (Y_Coord < 239))
      {
        /* Fila 3, columna 1 */
        if((X_Coord > 0) && (X_Coord < 106) && busy_vector[6] == 0){  //Coordenadas segmento 6
            player = 1;
            X_vector[6] = 1;
        }
      /* Fila 3, columna 2 */
      else if ((X_Coord > 106) && (X_Coord < 211) && busy_vector[7] == 0){ //Coordenadas segmento 7
            player = 1;
            X_vector[7] = 1;
        }
        /* Fila 3, columna 3 */
      else if (((X_Coord > 211) && (X_Coord < 319)&& busy_vector[8] == 0) ){ //Coordenadas segmento 8
            player = 1;
            X_vector[8] = 1;
        }
      }
      ...
      if (player == 1){
        ...
        output_vector = (X_vector[8]<<8)+(X_vector[7]<<7)+(X_vector[6]<<6)+(X_vector[5]<<5)+(X_vector[4]<<4)+(X_vector[3]<<3)+(X_vector[2]<<2)+(X_vector[1]<<1)+(X_vector[0]);
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------  
  else if (player == 1){ //Si el jugador activo es cero
...
void test(bool _vector[], bool _player){
  ...
  if ((_vector[0]  && _vector[1] && _vector[2] ) || (_vector[3] && _vector[4] && _vector[5]) || (_vector[6] && _vector[7] && _vector[8]) || (_vector[0] && _vector[3] && _vector[6]) || (_vector[1] && _vector[4] && _vector[7]) || (_vector[2] && _vector[5] && _vector[8]) || (_vector[4] && _vector[0] && _vector[8]) || (_vector[4] && _vector[2] && _vector[6]))
  {
    ...
    if (_player == 0){
      //Si el jugador activo es cero, imprime mensaje de perdedor
      ...
      //Si el jugador activo es equis, imprime mensaje de ganador
      myGLCD.setColor(0, 0, 0);
